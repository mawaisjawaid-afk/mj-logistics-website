...
async function getNextEstimateVersion(linkedBookingId) {
  if (!linkedBookingId) return 1;
  ...
  return Number(data[0].estimate_version || 0) + 1;
}

async function updateLinkedBookingAfterEstimate({
  linkedBookingId,
  estimateId,
  estimateCode,
}) {
  ...
  const nextEstimateCount = Number(booking?.estimate_count || 0) + 1;

  const { error: bookingUpdateError } = await supabase
    .from("internal_bookings")
    .update({
      linked_estimate_code: estimateCode,
      latest_estimate_id: estimateId,
      latest_estimate_code: estimateCode,
      estimate_count: nextEstimateCount,
...
function buildEmailSummaryHtml({
  ...
  blocks.push(
    buildSectionHtml("Estimate Information", [
      ...
      { label: "Generated On", value: generatedAtText },
    ])
  );
...
function drawBottomNotice({ page, y, noticeText, fonts }) {
  const { width } = page.getSize();
  ...
  drawCenteredText(page, noticeText, width / 2, y - 17, {
    font: fonts.regular,
    size: 6.8,
...
export async function POST(req) {
  try {
    ...
    console.log("save-internal-estimate incoming body:", {
      ...
      deliveryLon: body.deliveryLon,
      suggestedVehicle: body.suggestedVehicle,
      manualPrice: body.manualPrice,
      hasManualOverride: body.hasManualOverride,
    });
    ...
    const [routeData, vehiclesResult, versionNumberResult] = await Promise.all([
      ...
      supabase
        ...
        .eq("is_active", true),
      getNextEstimateVersion(linkedBookingId),
    ]);

    const distanceKm = routeData.distanceKm;
    const routeTimeHours = routeData.timeHours;
    const estimateVersion = versionNumberResult;
    ...
    const insertPayload = {
      ...
      linked_booking_code: linkedBookingCode,
    ...
    const estimateId = insertedEstimate.id;

    const BASE_NUMBER = 423000;
    const estimateCodeNumber = BASE_NUMBER + Number(estimateId);
    const estimateCode = `EST-${estimateCodeNumber}`;

    const { error: estimateCodeUpdateError } = await supabase
      .from("estimates")
      .update({ estimate_code: estimateCode })
      .eq("id", estimateId);

    if (estimateCodeUpdateError) throw estimateCodeUpdateError;

    await updateLinkedBookingAfterEstimate({
      linkedBookingId,
      estimateId,
      estimateCode,
    });
    ...
    try {
      ...
      console.error("Odoo webhook failed:", odooError);
    ...
    const estimatesTeamEmailHtml = buildInternalEmailHtml({
      title: "New Internal Estimate Generated",
      subtitle: "A new internal estimate has been generated from the booking flow.",
      summaryHtml: commonSummaryHtml,
      footerNote:
        "Internal estimate PDF and vendor requirement PDF are attached with this email.",
    });
    ...
    const operationsReviewHtml = `
      <div style="margin:0 0 18px 0;padding:16px;border:1px solid #fdba74;border-radius:14px;background:#fff7ed;">
        ...
        <div style="font-size:13px;color:#7c2d12;line-height:22px;">
          Please review vehicle recommendation, route assumptions, loading date, documents, and commercial feasibility.
          If this estimate is approved, please confirm by replying to this same email thread.
          If any revision is required, please reply to this same email thread with the required changes.
        </div>
      </div>
    `;
    ...
    const operationsActionHtml = `
      <div style="font-size:13px;color:#374151;line-height:22px;text-align:left;">
        ...
        • If approved, reply to this email and confirm that the estimate may be shared with the customer.<br />
        • If changes are required, reply to this email thread with the required revisions.
      </div>
    `;

    const operationsEmailHtml = buildInternalEmailHtml({
      title: "Operations Review Required",
      subtitle: "Please review this estimate before it is shared with the customer.",
    ...
    const estimateAttachments = [
      ...
      {
        filename: `${estimateCode}-vendor-requirement.pdf`,
    ...
    if (estimateRecipients.length > 0) {
      emailJobs.push(
        resend.emails.send({
          ...
          subject: `[INTERNAL ESTIMATE] ${estimateCode}`,
    ...
    if (operationsRecipients.length > 0) {
      emailJobs.push(
        resend.emails.send({
          ...
          subject: `[OPERATIONS REVIEW REQUIRED] ${estimateCode}`,
    ...
    if (finalEstimateFlagsError) throw finalEstimateFlagsError;
    ...
    console.error("save-internal-estimate API error:", err);
